...
where plants[i] is the amount of water the ith plant needs, and an integer capacity representing the watering can capacity, 
...
class Solution {
    public static int wateringPlants(int[] plants, int capacity) {
        ...
        int cap = capacity;
        for(int i =0;i<l;i++){
//        	System.out.println(" needed "+plants[i]+" at "+ i+ " capacity before : "+ cap);
            if(cap>=plants[i]){
            	steps+= i-current;
                cap -= plants[i];
            }else{
            ...
            current = i;
//            System.out.println(" needed "+plants[i]+" at "+ i+ " capacity after : "+ cap);
        }
        return steps;
    }
}
